...
if predict_button:
    ...
    input_df[['Age', 'Tumor Size']] = scaler.transform(input_df[['Age', 'Tumor Size']])
    ...
    if model_choice == 'Cox Proportional Hazards':
        ...
        st.subheader(f'Predicted Mean Survival Time: {predicted_mean_survival_time:.2f} months')
        ...
        st.subheader(f'Predicted Mean Survival Time: {predicted_means_tree[0]:.2f} months')
        ...
        st.subheader(f'Predicted Mean Survival Time: {predicted_means_rsf[0]:.2f} months')
        ...
        st.subheader(f'Predicted Mean Survival Time: {predicted_means_gbm[0]:.2f} months')
        ...
        # Predict survival function using survival tree
        survival_function_st = stacking_model.predict_survival_function(input_df)
        # Function to calculate mean survival time
        def mean_survival_time(surv_funcs):
            mean_times = []
            for surv_func in surv_funcs:
        ...
        # Calculate mean survival time for rsf
        predicted_means_st = mean_survival_time(survival_function_st)
        st.subheader(f'Predicted Mean Survival Time: {predicted_means_gbm[0]:.2f} months')
        
        # Visualize the survival curve
        st.subheader('Survival Curve')
        fig, ax = plt.subplots()
        for surv_func in survival_function_st:
            ax.step(surv_func.x, surv_func.y, where='post')
        ax.set_xlabel('Time (months)')
        ax.set_ylabel('Survival Probability')
        ax.set_title('Predicted Survival Curve')
